...
- **chatId**: `sessions/telegram-chat-id.txt` (plaintext, `.gitignore` 의 `sessions/` 룰로 자동 제외)
...
|------|------|
| 상태 확인 | `pm2 status` |
| 감시 재시작 | `pm2 restart ict-watcher` |
| 감시 중지 | `pm2 stop ict-watcher` |
| 감시 시작 | `pm2 start ict-watcher` |
| 로그 | `pm2 logs ict-watcher` |
| 수동 테스트 | `TELEGRAM_NOTIFY=1 node scripts/watcher.js` |
| 대시보드 무중단 재시작 | `pm2 reload trading-dashboard` |
| 대시보드 로그 | `pm2 logs trading-dashboard` |

## 트레이딩 파라미터 설정 (Hot-reload)
- **파일**: `sessions/trade.env` (git-ignored, 즉시 편집 가능)
- **변경 반영**: watcher 매 틱(60초) 자동 재읽기 — `pm2 restart` 불필요
- **설정 항목**: `LEVERAGE`, `MARGIN_USD`, `NOTIONAL_USD`, `BINANCE_API_KEY/SECRET`, `BYBIT_API_KEY/SECRET`
- **템플릿**: 루트 `trade.env.example` 참조. API 키 비워두면 `sessions/*.txt` fallback 사용
- **파이프라인**: `watcher.js`의 `_mergeEnv(traderConfig, loadTradeEnv())`가 매 틱 merged cfg 생성 → `judgeSignal(signal, cfg)` + `execute(signal, verdict, cfg)` 에 전달
